...
/**
 * Set of utility operations on the H2 database. The H2Util class is a
 * singleton and provides a single instance through the getInstance()
 * method. The utility class guarantees that only a single connection
 * to the database is created for all users of the H2Util class.
 * 
 * @author Paul Osborne
 */
public class H2Util {
    ...
    /**
     * Private constructor for the singleton. Initializes the state of
     * the utility class.
     */
    private H2Util() {
        try {
            Class.forName("org.h2.Driver"); // load H2 driver into
                                            // memory
        } catch (ClassNotFoundException ex) {
            System.err
                    .println("Could not load H2 driver into memory");
        }
        this.conn = null;
    }
     ...
     * 
     * @param dbLocation
     *            The location of the database that should be used.
     *            This can either be an absolute path (C:\mango.db) or
     *            a relative path (~/testing.db OR ./mangotesting.db).
     */
     ...
     * @throws SQLException
     *             in the case that there is an error creating a
    ...
    /**
     * Return a connection to the database. The class is set up such
     * that only one connection to the database will be established.
     * 
    ...
    /**
     * @param values
     *            The values that should be quoted and comma
     *            separated.
    ...
    /**
     * Mini application for resetting the schema on a database to the
     * newest state. This will remove all data in the database.
     * 
     * @param args
    ...
    public static void main(String[] args) {
        ...
        System.out
                .println("This utility will clear and reset schema on DB");
