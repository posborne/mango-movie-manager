...
/**
 * Custom JTree for navigation stuff. A lot of this is handled in the
 * models, but there was enough with all the listeners and such to
...
public class NavigatorJTree extends JTree {
    ...
    /**
     * Construct the JTree with the base model and set things so that
     * this stinker shows up how we want it to.
     */
    public NavigatorJTree() {
        this.setModel(new BaseTreeModel());
        this.setRootVisible(false);
        this
                .addTreeSelectionListener(new NavigatorTreeSelectionListener(
                        this));
    ...
    public class NavigatorPopupListener implements MouseListener {
        ...
        private void maybeShowPopup(MouseEvent e) {
            if (e.isPopupTrigger()) {
                int treeRow = NavigatorJTree.this.getRowForLocation(e
                        .getX(), e.getY());
                TreePath treePath = NavigatorJTree.this
                        .getPathForLocation(e.getX(), e.getY());
                if (treeRow > 0) {
                    DefaultMutableTreeNode tn = (DefaultMutableTreeNode) treePath
                            .getLastPathComponent();
                    if (tn.isLeaf()) {
                        MangoMutableTreeNode mangoTN = (MangoMutableTreeNode) tn;
                        JPopupMenu popup = mangoTN.getPopupMenu();
                        if (popup != null) {
                            popup.show((JComponent) e.getSource(), e
                                    .getX(), e.getY());
                        }
                    }
                }
