...
/**
 * Tree node for making the main table display the set of all borrowed movies.
 * 
...
public class BorrowedMoviesMutableTreeNode extends DefaultMutableTreeNode
	...
	/** 
	 * Set the table to display borrowed movies.  Borrowed movies are movies
	...
	public void doYourThing(Mango mangoPanel) {
		...
		if (ownerId == -1) {
			SetOwnerDialog d = new SetOwnerDialog(UIController.getInstance()
					.getMango(), true);
			d.setLocationRelativeTo(UIController.getInstance().getMango());
			d.setVisible(true);
			ownerId = UIController.getInstance().getOwnerId();
		}
		String sql = "SELECT id FROM movie" + "	WHERE borrower_id = " + ownerId;
