...
/**
 * Tree node for representing a launching the action of display a list.
 * 
 * @author Paul Osborne
 */
public class ListMutableTreeNode extends DefaultMutableTreeNode implements
	...
	/**
	 * Get a popup menu with actions on the list.  Right now the options
	 * are:
	 *  - Delete List
	 *  - Rename List
	 */
	...
	public JPopupMenu getPopupMenu() {
			...
			public void actionPerformed(ActionEvent e) {
				if (JOptionPane.showConfirmDialog((Component) e.getSource(),
						"Are you sure you want to remove the list \""
								+ listLabel + "\"?", "Confirm List Deletion",
		...
		renameListItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String newName = JOptionPane.showInputDialog((Component) e
						.getSource(), "Rename \"" + listLabel + "\" to?",
						"Rename List", JOptionPane.PLAIN_MESSAGE);
				if (newName != null) {
					MangoController.getInstance()
							.renameList(listLabel, newName);
					listLabel = newName;
					TreePath path = new TreePath(ListMutableTreeNode.this
							.getPath());
					UIController.getInstance().getNavigatorTree().getModel()
							.valueForPathChanged(path, newName);
				}
			}
		});
