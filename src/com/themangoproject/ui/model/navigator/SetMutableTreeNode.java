...
public class SetMutableTreeNode extends DefaultMutableTreeNode implements
	...
	/**
	 * Make the table display the movies in the set this node represents.
	 */
	@Override
	...
	/**
	 * @return a popup menu with actions relating the the set this node
	 *         represents
	 */
	@Override
		...
		renameSetItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String newName = JOptionPane.showInputDialog((Component) e
						.getSource(), "Rename \"" + setLabel + "\" to?",
						"Rename Set", JOptionPane.PLAIN_MESSAGE);
				if (newName != null) {
					MangoController.getInstance().renameSet(setLabel, newName);
					setLabel = newName;
					TreePath path = new TreePath(SetMutableTreeNode.this
							.getPath());
