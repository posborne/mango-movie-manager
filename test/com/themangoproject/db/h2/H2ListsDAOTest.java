...
public class H2ListsDAOTest {
    ...
    public void testGetMoviesInList() {
        ...
        List<Movie> marathonMovies = H2ListsDAO.getInstance()
                .getMoviesInList("Action Marathon");
        assertEquals("Die Hard", marathonMovies.get(0).getTitle());
        assertEquals("Die Hard: With a Vengeance", marathonMovies
                .get(1).getTitle());
    }
    ...
    public void testReorderMoviesInList() {
        ...
        List<Movie> movies = H2ListsDAO.getInstance()
                .getMoviesInList("Action Marathon");
        ArrayList<Movie> newMoviesList = new ArrayList<Movie>();
        newMoviesList.add(movies.get(1));
        newMoviesList.add(movies.get(0));

        H2ListsDAO.getInstance().reorderMoviesInList(
                "Action Marathon", newMoviesList);

        List<Movie> moviesAgain = H2ListsDAO.getInstance()
                .getMoviesInList("Action Marathon");
        assertEquals("Die Hard: With a Vengeance", moviesAgain.get(0)
                .getTitle());
    ...
    public void testAddMovieToList() {
        TestUtility.executeInserts();
        H2ListsDAO.getInstance().addMovieToList(
                "Action Marathon",
                H2ListsDAO.getInstance().getMoviesInList(
                        "Action Marathon").get(0));
        ...
        assertEquals("Die Hard: With a Vengeance", movies.get(1)
                .getTitle());
