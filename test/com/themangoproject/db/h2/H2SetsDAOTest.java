...
public class H2SetsDAOTest {
	...
	public void testGetMoviesInSet() {
		...
		List<Movie> movies = H2SetsDAO.getInstance().getMoviesInSet("Die Hard Series");
	...
	public void testRemoveMovieFromSet() {
		...
		dieHard.setId(1);
		H2SetsDAO.getInstance().removeMovieFromSet("Die Hard Series", dieHard);
		
		List<Movie> movies = H2SetsDAO.getInstance().getMoviesInSet("Die Hard Series");
		assertEquals(1, movies.size());
		assertEquals("Die Hard: With a Vengeance", movies.get(0).getTitle());
	}
